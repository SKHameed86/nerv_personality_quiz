...
const PHASES=[{"name":"THE SELF","subtitle":"Who are you when nobody else is defining you?","color":"#FF6600","terminal":"MELCHIOR"},{"name":"OTHERS","subtitle":"How do you understand connection, difference and responsibility?","color":"#00FFCC","terminal":"BALTHASAR"},{"name":"PRESSURE","subtitle":"What happens when certainty and control begin to fail?","color":"#39FF14","terminal":"CASPER"},{"name":"IDENTITY","subtitle":"What remains when circumstances, memory and expectation change?","color":"#CC44FF","terminal":"MAGI TRIAD"},{"name":"SYNCHRONISATION","subtitle":"The deepest conflicts between attachment, principle and choice.","color":"#FF3333","terminal":"MAGI TRIAD"}];
...
function showPhase(index,after){
  const p=PHASES[phaseFor(index)];
  document.documentElement.style.setProperty('--phase',p.color);
  $('phaseNumber').textContent=`PHASE ${String(phaseFor(index)+1).padStart(2,'0')}`;
...
$('restartBtn').onclick=()=>{
  location.hash='';current=0;answers=Array(QUESTIONS.length).fill(null);
  showPhase(0,()=>{show('quiz');renderQuestion();});
};
